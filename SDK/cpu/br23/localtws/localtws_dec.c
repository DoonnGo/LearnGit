...
extern struct audio_wireless_sync *audio_localtws_sync_open(u8 entry_select);
...
{
    audio_decoder_task_del_wait(&decode_task, &localtws_dec->wait);
...
static int localtws_dec_start()
{
    ...
    // 打开file解码器
    err = localtws_decoder_open(&dec->local_dec, &decode_task);
    if (err) {
        goto __err1;
    }
    ...
    dec->sync = audio_localtws_sync_open(0);
...
int localtws_dec_open(u32 value)
{
    ...
    localtws_media_get_info((u8 *)&value, &fmt);
