...
const u16 User_vol_table[64] = {
    ...
    1747 * 29 / 30,
    1747 * 30 / 30,
    (16384-1747)*(31-30)/33+1747,
    (16384-1747)*(32-30)/33+1747,
    (16384-1747)*(33-30)/33+1747,
    (16384-1747)*(34-30)/33+1747,
    (16384-1747)*(35-30)/33+1747,
    (16384-1747)*(36-30)/33+1747,
    (16384-1747)*(37-30)/33+1747,
    (16384-1747)*(38-30)/33+1747,
    (16384-1747)*(39-30)/33+1747,
    (16384-1747)*(40-30)/33+1747,
    (16384-1747)*(41-30)/33+1747,
    (16384-1747)*(42-30)/33+1747,
    (16384-1747)*(43-30)/33+1747,
    (16384-1747)*(44-30)/33+1747,
    (16384-1747)*(45-30)/33+1747,
    (16384-1747)*(46-30)/33+1747,
    (16384-1747)*(47-30)/33+1747,
    (16384-1747)*(48-30)/33+1747,
    (16384-1747)*(49-30)/33+1747,
    (16384-1747)*(50-30)/33+1747,
    (16384-1747)*(51-30)/33+1747,
    (16384-1747)*(52-30)/33+1747,
    (16384-1747)*(53-30)/33+1747,
    (16384-1747)*(54-30)/33+1747,
    (16384-1747)*(55-30)/33+1747,
    (16384-1747)*(56-30)/33+1747,
    (16384-1747)*(57-30)/33+1747,
    (16384-1747)*(58-30)/33+1747,
    (16384-1747)*(59-30)/33+1747,
    (16384-1747)*(60-30)/33+1747,
    (16384-1747)*(61-30)/33+1747,
    (16384-1747)*(62-30)/33+1747,
    (16384-1747)*(63-30)/33+1747,
};
...
#if 0///2020-11-13 源码数字音量级数调节不完善，自修改
...
#endif
#else
    ///2020-11-13 源码数字音量级数调节不完善，自修改
    gain_l = gain_l>63?63:gain_l;
    gain_r = gain_r>63?63:gain_r;
    gain_rl = gain_rl>63?63:gain_rl;
    gain_rr = gain_rr>63?63:gain_rr;
    
    if(app_var.volmcu_type == 0)
    {
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(0), gain_l ? 31 : 0, fade);
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(1), gain_r ? 31 : 0, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(0), User_vol_table[gain_l], fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(1), User_vol_table[gain_r], fade);

#if ((TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR) \
    || (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_DUAL_LR_DIFF))
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(2), gain_rl ? 31 : 0, fade);
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(3), gain_rr ? 31 : 0, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(2), User_vol_table[gain_rl], fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(3), User_vol_table[gain_rr], fade);
#endif
    }
    else if(app_var.volmcu_type == 1)
    {
        dsp_module_channel_set_vol(1,gain_l);
        dsp_module_channel_set_vol(2,gain_r);
        dsp_module_channel_set_vol(3,gain_rl);
        dsp_module_channel_set_vol(4,gain_rr);

        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(0), 31, fade);
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(1), 31, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(0), DEFAULT_DIGTAL_VOLUME, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(1), DEFAULT_DIGTAL_VOLUME, fade);

#if ((TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR) \
    || (TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_DUAL_LR_DIFF))
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(2), 31, fade);
        audio_dac_vol_set(TYPE_DAC_AGAIN, BIT(3), 31, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(2), DEFAULT_DIGTAL_VOLUME, fade);
        audio_dac_vol_set(TYPE_DAC_DGAIN, BIT(3), DEFAULT_DIGTAL_VOLUME, fade);
#endif
    }
#endif
...
///4声道独立调节:2020-11-10
/***********************************************************************************/
    ///计算在bal和fad减益后的4声道音量比数
    u8 gain_l_lv = 7, gain_r_lv = 7, gain_f_lv = 7, gain_rr_lv = 7;
    u8 gain_fl, gain_fr, gain_rl, gain_rr;
    u8 tmp = 0;
    if(app_var.bal_vol >= 0) gain_l_lv = gain_l_lv - app_var.bal_vol;
                        else gain_r_lv = gain_r_lv + app_var.bal_vol;
    if(app_var.fad_vol >= 0) gain_rr_lv = gain_rr_lv - app_var.fad_vol;
                        else gain_f_lv = gain_f_lv + app_var.fad_vol;
    //printf("4channel:f:%d,r:%d,l:%d,r:%d\n",gain_f_lv,gain_rr_lv,gain_l_lv,gain_r_lv);

    ///对比前和左,取较小者赋值到前左声道
    (gain_f_lv > gain_l_lv)?(gain_fl = gain_l_lv):(gain_fl = gain_f_lv);
    (gain_f_lv > gain_r_lv)?(gain_fr = gain_r_lv):(gain_fr = gain_f_lv);
    (gain_rr_lv > gain_l_lv)?(gain_rl = gain_l_lv):(gain_rl = gain_rr_lv);
    (gain_rr_lv > gain_r_lv)?(gain_rr = gain_r_lv):(gain_rr = gain_rr_lv);
    //printf("4channel:fl:%d,fr:%d,rl:%d,rr:%d\n",gain_fl,gain_fr,gain_rl,gain_rr);

    ///得出的4声道音量比数等比缩小音量值
//    u16 volume = left_gain * 16384 / get_max_sys_vol();
//    __this->digital_volume_fl = gain_fl * volume / 7;
//    __this->digital_volume_fr = gain_fr * volume / 7;
//    __this->digital_volume_rl = gain_rl * volume / 7;
//    __this->digital_volume_rr = gain_rr * volume / 7;
    //printf("4channel:fl:%d,fr:%d,rl:%d,rr:%d\n",__this->digital_volume_fl,__this->digital_volume_fr,__this->digital_volume_rl,__this->digital_volume_rr);

//    audio_dac_set_L_digital_vol(&dac_hdl, __this->digital_volume_fl);
//    audio_dac_set_R_digital_vol(&dac_hdl, __this->digital_volume_fr);
//    audio_dac_set_RL_digital_vol(&dac_hdl, __this->digital_volume_rl);
//    audio_dac_set_RR_digital_vol(&dac_hdl, __this->digital_volume_rr);
    u16 volume = left_gain;
    gain_fl = volume * gain_fl / 7;
    gain_fr = volume * gain_fr / 7;
    gain_rl = volume * gain_rl / 7;
    gain_rr = volume * gain_rr / 7;
    
    bsp_app_bal_status();//blesend
    bsp_app_fad_status();//blesend

    audio_vol_set(gain_fl, gain_fr, gain_rl, gain_rr, fade);
/***********************************************************************************/
#else
...
//        }
//        volume = app_var.aec_dac_gain * app_var.call_volume / get_max_sys_vol();
        ///2020-12-4
    ...
    //app_audio_volume_change();    ///2020-12-5 取消音量记忆，排除复位问题
}
...
extern u16 User_pa_unmute_timeid;
u16 User_set_vol_unmute_timeid = 0;
void User_set_vol_unmute(void)
{
    User_set_vol_unmute_timeid = 0;
    USER_MUTE_DISABLE();
}
void asm_nop_set(u16 tik)
{
    for(;tik>1;tik--)
    {
        asm("nop");
...
void app_audio_volume_down(u8 value)
{
    ...
    if(User_pa_unmute_timeid == 0)
    {
//        if(User_set_vol_unmute_timeid)
//            sys_timer_re_run(User_set_vol_unmute_timeid);
//        else
            User_set_vol_unmute_timeid = sys_timeout_add(NULL, User_set_vol_unmute, 100);
    }
}
...
{
    app_var.bal_vol = volume;
    app_audio_set_volume(__this->state, app_audio_get_volume(__this->state), 1);
}

s8 User_get_bal_volume(void)
{
    return app_var.bal_vol;
}
...
s8 User_bal_volume_dir(u8 dir)
{
    if(dir)
    {
        if(app_var.bal_vol < 7)
        {
            app_var.bal_vol++;
            User_bal_volume_set(app_var.bal_vol);
        }
    }
    else
    {
...
{
    app_var.fad_vol = volume;
    app_audio_set_volume(__this->state, app_audio_get_volume(__this->state), 1);
}

s8 User_get_fad_volume(void)
{
    return app_var.fad_vol;
}
...
s8 User_fad_volume_dir(u8 dir)
{
    if(dir)
    {
        if(app_var.fad_vol < 7)
        {
            app_var.fad_vol++;
            User_fad_volume_set(app_var.fad_vol);
        }
    }
    else
    {
