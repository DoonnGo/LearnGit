...
#if 0//(TCFG_AUDIO_DAC_CONNECT_MODE == DAC_OUTPUT_FRONT_LR_REAR_LR) ///2020-12-5 使用4声道时,这里不能设置为4,否则通话声会严重变形
...
surround_hdl *surround_open_demo(u8 ch_num)
{
    ...
    parm.channel = ch_num;
    parm.surround_effect_type = EFFECT_3D_PANORAMA;
    surround = audio_surround_open(&parm);

    clock_add(DEC_3D_CLK);
    //suttt = sys_timer_add(surround, surround_switch_test, 10000);
#endif
    return surround;
}
/*----------------------------------------------------------------------------*/
