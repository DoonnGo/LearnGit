...
int linein_dec_start()
{
    ...
    dec->eq_drc = linein_eq_drc_open(dec->pcm_dec.sample_rate, dec->pcm_dec.ch_num);
#if TCFG_EQ_DIVIDE_ENABLE
    dec->eq_drc_rl_rr = linein_rl_rr_eq_drc_open(dec->pcm_dec.sample_rate, dec->pcm_dec.ch_num);
    if (dec->eq_drc_rl_rr) {
    ...
    /* audio_stream_add_entry(entries[entry_cnt-3], &dec->synthesis_ch_rl_rr.entry); */
#endif
