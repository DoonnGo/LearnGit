...
static void reset_pcm_sample_rate(u16 sr);
...
static u16 get_pcm_sample_rate(void)
{
...
int spdif_dec_start(void)
{
...
#if TCFG_EQ_DIVIDE_ENABLE
    if (spdif_dec_hdl->eq_drc_rl_rr) {
        entries[entry_cnt++] = &spdif_dec_hdl->synthesis_ch_fl_fr.entry;
    } else {
    ...
    if (spdif_dec_hdl->eq_drc_rl_rr) { //接在eq_drc的上一个节点
        audio_stream_add_entry(entries[entry_cnt - 4], &spdif_dec_hdl->eq_drc_rl_rr->entry);
