...
#if (RECORDER_MIX_EN)
    ///送录音数据流
    effect->rec_hdl = stream_entry_open(effect, mic_effect_record_stream_callback);
#endif
...
// 数据流串联
    ...
    if (effect->rec_hdl) {
        //从变采样前一节点分流
        audio_stream_add_entry(entries[entry_cnt - 3], &effect->rec_hdl->entry);
    }
#endif
...
/*----------------------------------------------------------------------------*/
/**@brief    mic_effect_cal_coef 混响高低音 调节接口
   @param    filtN:0 喊麦模式， sw:0关喊麦  1开喊麦
   @param    filtN:1 低音调节  gainN：调节的增益，没效果文件时 ,范围0~10
   @param    filtN:2 高音调节  gainN：调节的增益，没效果文件时 ,范围0~10
   @return
   @note    混响高低音调节
*/
/*----------------------------------------------------------------------------*/
...
/**@brief    mic_effect_cal_coef 混响高低音 调节接口
   ...
   @param    filtN:MIC_EQ_MODE_LOW_SOUND   低音调节  gainN：调节的增益，没效果文件时 ,范围0~10
   @param    filtN:MIC_EQ_MODE_HIGH_SOUND  高音调节  gainN：调节的增益，没效果文件时 ,范围0~10
   @return
   @note     混响高低音调节
*/
/*----------------------------------------------------------------------------*/
