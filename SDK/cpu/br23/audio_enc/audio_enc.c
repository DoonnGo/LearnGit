...
int audio_mic_open(struct adc_mic_ch *mic, u16 sample_rate, u8 gain)
{
    ...
    if (!ladc_var.adc_buf) {
        ...
        if (TCFG_LINEIN_LR_CH & (0x2A)) {
            ladc_var.ladc_ch_mark |= BIT(1);
            ladc_var.ladc_ch_num++;
        }
#endif
        ladc_var.adc_buf = (s16 *)zalloc(ladc_var.ladc_ch_num * LADC_IRQ_POINTS * 2 * LADC_BUF_NUM);
    ...
    if (ladc_var.states == 0) {
        ...
        if (ladc_var.ladc_ch_num == 1) {
            audio_adc_mic_set_buffs(&ladc_var.mic_ch, ladc_var.adc_buf,  ladc_var.ladc_ch_num * LADC_IRQ_POINTS * 2, LADC_BUF_NUM);
        } else {
            audio_adc_set_buffs(&ladc_var.linein_ch, ladc_var.adc_buf,  ladc_var.ladc_ch_num * LADC_IRQ_POINTS * 2, LADC_BUF_NUM);
