...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_ae01_01_VALUE_HANDLE:
        ...
        if (app_send_user_data_check(buffer_size)) {
            extern void bsp_custom_app_process(u8 *ptr, u8 len);
            bsp_custom_app_process(buffer, buffer_size);
//            app_send_user_data(ATT_CHARACTERISTIC_ae01_01_VALUE_HANDLE, buffer, buffer_size, ATT_OP_AUTO_READ_CCC);
        }
        break;
...
static int make_set_adv_data(void)
{
    ...
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, buff, 8); ///2020-12-19 公司名,用于APP识别
