...
/*限幅器上限*/
const int CONST_LIMITER_THR = -10000; /*-12000 = -12dB,放大1000倍,(-10000参考)*/
/*小于CONST_NOISE_GATE的当成噪声处理,防止清0近端声音*/
const int CONST_NOISE_GATE = -30000; /*-12000 = -12dB,放大1000倍,(-30000参考)*/
/*低于噪声门限阈值的增益 */
const int CONST_NOISE_GAIN = (int)(0 << 30); /*(0~1)*2^30*/
/*单工连续清0的帧数*/
...
#define AEC_OUTPUT_SRC_ENABLE	1
#if AEC_OUTPUT_SRC_ENABLE
...
static u16 sw_src_output_rate = 0;
static int (*sw_src_output_handler)(u8 *data, u32 len) = NULL;
int audio_aec_sw_src_outsr(u16 sample_rate)
{
    g_printf("aec sw outsr: %d\n", sample_rate);
    sw_src_output_rate = sample_rate;
    return 0;
}
int audio_aec_sw_src_out_handler(void *handler)
{
    sw_src_output_handler = handler;
    return 0;
}
...
int sw_src_init()
{
    ...
    rs_para_obj.new_outsample = sw_src_output_rate;
...
static int audio_aec_output(s16 *data, u16 len)
{
    ...
    s16 *outdat = data;
#if AEC_OUTPUT_SRC_ENABLE
    if (sw_src_api && sw_src_buf && sw_src_output_handler) {
...
#if AEC_OUTPUT_SRC_ENABLE
    if (sw_src_output_handler != NULL) {
        wlen = sw_src_output_handler(outdat, outlen);
    } else
#endif
    {
        wlen = cbuf_write(&aec_hdl->output_cbuf, outdat, outlen);
    }
...
#if AEC_OUTPUT_SRC_ENABLE
    if (sw_src_output_rate != 0) {
        printf("audio_aec src init\n");
        sw_src_init();
    }
#endif
...
#if AEC_OUTPUT_SRC_ENABLE
    sw_src_output_rate = 0;
    sw_src_output_handler = NULL;
#endif
}
