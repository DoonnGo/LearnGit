...
//struct sys_time last_rtc_time = {0};   ///实时记录时间用于set time;
...
static void ui_lcdseg_show_curtime(void *hd, u8 Hour, u8 Min)
{
    LCD_API *dis = (LCD_API *)hd;
    u8 tmp_buf[5] = {0};
...
static void ui_lcdseg_show_set_time(void *hd, u8 Hour, u8 Min, u8 Flash_sta)
{
    LCD_API *dis = (LCD_API *)hd;
    u8 tmp_buf[5] = {0};
    itoa2(Hour, (u8 *)&tmp_buf[0]);
...
static int ui_lcdseg_show_RTC_user(void *hd, struct ui_rtc_display *rtc)
{
    int ret = true;
    if (rtc == NULL) {
        return false;
...
///2020-11-25 ACC时不显示
static void ui_lcdseg_show_acc_nothing(void *hd)
{
    LCD_API *dis = (LCD_API *)hd;
...
static void ui_rtc_main(void *hd, void *private) //主界面显示
{
    ...
    {
        ui_lcdseg_show_acc_nothing(hd);
        return;
    }

    dev_ioctl(__this->dev_handle, IOCTL_GET_SYS_TIME, (u32)&current_time);

//    memcpy(&last_rtc_time, &current_time, sizeof(current_time));  ///2020-11-21 记录rtc值用于set time
    ...
    printf("rtc_read_sys_time: %d-%d-%d %d:%d:%d\n",
           ...
           current_time.sec);
#endif
    ui_lcdseg_show_curtime(hd, current_time.hour, current_time.min);
}
...
static int ui_rtc_user(void *hd, void *private, u8 menu, u32 arg)//子界面显示 //返回true不继续传递 ，返回false由common统一处理
{
    ...
    switch (menu) {
#if 0   ///2020-11-21 取消原来的显示方案
    case MENU_ALM_SET:
    case MENU_RTC_SET:
        ui_lcdseg_show_RTC_user(hd, &(__this->ui_rtc));
        break;
#endif
    case MENU_RTC_SET_HOUR:
    case MENU_RTC_SET_MIN:
//        ui_lcdseg_show_set_time(hd, last_rtc_time.hour, last_rtc_time.min, menu-MENU_RTC_SET_HOUR);
        break;
    default:
        ret = false;
        break;
