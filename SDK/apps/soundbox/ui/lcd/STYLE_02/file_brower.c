...
static int file_select_enter(int from_index)
{
    ...
    if (__this->attr[i].attr == F_ATTR_DIR) { //判断是不是文件夹文件属性
        if (!from_index) {
            ...
            __this->fs = fscan_enterdir(__this->fs, __this->text_list[i].fname);
        }

        if (!__this->fs) {
            ui_hide(MUSIC_FILE_LAYOUT);
            ui_hide(MUSIC_MENU_LAYOUT);
            ui_redraw(MUSIC_MENU_LAYOUT);
            return false;
        }
...
__find:
    ...
    if (f) {
        __this->text_list[i].len = fget_name(f, __this->text_list[i].fname, TEXT_NAME_LEN);
        __this->text_list[i].unicode = fget_name_type(__this->text_list[i].fname, __this->text_list[i].len);
...
static int file_browse_enter_onchane(void *ctr, enum element_change_event e, void *arg)
{
    ...
    case ON_CHANGE_INIT:
        ...
        if (!__this->fs) {
            ...
            void *dev = dev_manager_find_spec("sd0", 1);
            if (!dev) {
                break;
            }
