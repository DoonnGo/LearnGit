...
static void fm_read_info_init(void)
{
    ...
    u8 show_channel = 0;
    if(__this->fm_freq_cur != get_fre_via_channel(__this->fm_freq_channel_cur)) //对比当前台号是否为当前频点
    {
        show_channel = 0x80;
    }

    if(__this->fm_freq_channel_cur > 12)
        band = 3;
    else if(__this->fm_freq_channel_cur > 6)
        band = 2;
    else   
        band = 1;
    bsp_app_fm_band_channel(band);//blesend

    ///2020-11-24 实时显示台号
    fm_manage_set_channel(__this->fm_freq_channel_cur|show_channel);
}
...
{
    ///2020-12-7 仅在FM主页(显示freq)时才刷新主页，避免显示闪烁
    //UI_REFLASH_WINDOW(true);
    UI_REFLASH_WINDOW(false);
...
static void __set_fm_station()
{
    ...
    ///实时显示台号
    fm_manage_set_channel(__this->fm_freq_channel_cur);
}
...
static void __fm_scan_all(void)
{
    ...
    if (fm_manage_set_fre(REAL_FREQ(__this->scan_fre))) {
        ...
        save_fm_point(REAL_FREQ(__this->scan_fre), __this->fm_freq_channel_cur);

        if(__this->fm_freq_channel_cur == 1)
        {
            fm_band_freq_buf[0] = REAL_FREQ(__this->scan_fre);
        }
        else if(__this->fm_freq_channel_cur == 7)
        {
            fm_band_freq_buf[1] = REAL_FREQ(__this->scan_fre);
        }
        else if(__this->fm_freq_channel_cur == 13)
        {
            fm_band_freq_buf[2] = REAL_FREQ(__this->scan_fre);
        }
...
static void __fm_semi_scan(void)//半自动收台
{
    ...
    if (fm_manage_set_fre(REAL_FREQ(__this->scan_fre))) {
        __this->fm_freq_cur  = __this->scan_fre;
        save_fm_point(REAL_FREQ(__this->scan_fre), 0);//保存当前频点
...
///中断搜台
void User_fm_scan_stop(void)
{
    switch(__this->scan_flag)
    {
    case 0:
        break;
    case SCANE_ALL:
        fm_scan_stop();
        break;
    case SEMI_SCANE_DOWN:
    case SEMI_SCANE_UP:
        fm_semi_stop();
        break;
    case CHANNELS_AUTOPLAY:
        fm_auto_play_select();
        break;
    }
    __fm_ui_reflash_main();
...
{
    if(__this->step_timer_id)
    {
        fm_prev_freq();
        if(__this->step_timer_id)
            sys_timer_re_run(__this->step_timer_id);
        return;
    }
    ...
    ///实时显示台号
    fm_manage_set_channel(__this->fm_freq_channel_cur|0x80);
}
...
///2020-11-24 按住方向键步进
static void __fm_step_timerout(void)
{
//    UI_SHOW_MENU(MENU_FM_STEP_OUT, 500, 0, NULL);
    __this->step_timer_id = 0;
}

void fm_prev_step_long(void)
{
    if (!__this || __this->scan_flag) {
        return;
    }
    ...
    fm_manage_set_channel(__this->fm_freq_channel_cur|0x80);
    UI_REFLASH_WINDOW(0);
//    UI_SHOW_MENU(MENU_FM_STEP_IN, 500, 0, NULL);

    __this->step_timer_id = sys_timeout_add(NULL, __fm_step_timerout, 3000);
}
void fm_prev_step_hold(void)
{
    if (!__this || __this->scan_flag) {
        return;
    }
...
void fm_auto_play_select(void)
{
    if (__this && __this->scan_flag == CHANNELS_AUTOPLAY) {
        __this->scan_flag = 0;
        //os_time_dly(1);
        fm_app_mute(0);
        __fm_reverb_resume();
...
static void __fm_auto_play(void)
{
    if (!__this || __this->scan_flag != CHANNELS_AUTOPLAY) {
        return;
    }

    if(__this->fm_freq_channel_cur < 18)
        __this->fm_freq_channel_cur++;
    else
        __this->fm_freq_channel_cur = 1;
    __set_fm_station();
    ...
    if(__this->fm_freq_channel_cur > 1)
        sys_timeout_add(NULL, __fm_auto_play, 5000);
}

void fm_auto_play_all_channel(void)
{
    if (!__this || __this->scan_flag) {
        User_fm_scan_stop();
        return;
    ...
    __this->scan_flag = CHANNELS_AUTOPLAY;
    //os_time_dly(1);
    __this->fm_freq_channel_cur = 1;
    __set_fm_station();
    __fm_reverb_resume();
...
void fm_prev_station()
{
    ...
    if (__this->fm_freq_channel_cur <= 1) {
        __this->fm_freq_channel_cur = 18;//__this->fm_total_channel;
    } else {
        __this->fm_freq_channel_cur -= 1;
    }
    __set_fm_station();
...
void fm_next_station()
{
    ...
    if (__this->fm_freq_channel_cur >= 18/*__this->fm_total_channel*/) {
        __this->fm_freq_channel_cur = 1;
    } else {
        __this->fm_freq_channel_cur += 1;
    }
...
void fm_set_band_channel(u8 channel)
{
    if (!__this || __this->scan_flag) {
        User_fm_scan_stop();
        return;
...
void fm_save_band_channel(u8 channel)
{
    if (!__this || __this->scan_flag) {
        User_fm_scan_stop();
        return;
    ...
    __this->fm_freq_channel_cur = channel;
    save_fm_point(REAL_FREQ(__this->fm_freq_cur), __this->fm_freq_channel_cur);

    __set_fm_station();
    __fm_ui_cur_station();
}

u16 fm_band_freq_buf[3]     = {8750, 8750, 8750};
u8  fm_band_channel_buf[3]  = {1, 7, 13};
void fm_next_band(void)
{
    if (!__this || __this->scan_flag) {
        User_fm_scan_stop();
        return;
    ...
    if(__this->fm_freq_channel_cur > 12)        //band3->1
    {
        fm_band_channel_buf[2] = __this->fm_freq_channel_cur;
        fm_band_freq_buf[2] = REAL_FREQ(__this->fm_freq_cur);

        __this->fm_freq_channel_cur = fm_band_channel_buf[0];
        freq = fm_band_freq_buf[0];
        band = 1;
    }
    else if(__this->fm_freq_channel_cur > 6)    //band2->3
    {
        fm_band_channel_buf[1] = __this->fm_freq_channel_cur;
        fm_band_freq_buf[1] = REAL_FREQ(__this->fm_freq_cur);

        __this->fm_freq_channel_cur = fm_band_channel_buf[2];
        freq = fm_band_freq_buf[2];
        band = 3;
    }
    else                                        //band1->2
    {
        fm_band_channel_buf[0] = __this->fm_freq_channel_cur;
        fm_band_freq_buf[0] = REAL_FREQ(__this->fm_freq_cur);

        __this->fm_freq_channel_cur = fm_band_channel_buf[1];
        freq = fm_band_freq_buf[1];
        band = 2;
    }

    //__set_fm_station();
    fm_app_mute(1);
    __this->fm_freq_cur = VIRTUAL_FREQ(freq);
    fm_manage_set_fre(REAL_FREQ(__this->fm_freq_cur));
    fm_app_mute(0);
