...
static const u8 app_task_list[] = {
    ...
    APP_LINEIN_TASK,
#endif
#if 0//TCFG_APP_RTC_EN  ///2020-11-23 RTC任务不在模式配置表内，只用clk按键调用显示时钟
    APP_RTC_TASK,
#endif
#if TCFG_APP_PC_EN
    APP_PC_TASK,
...
{
    ///2020-11-13 用于开机返回关机前的模式，如果是刚上电则直接去FM模式
    if(app_prev_task == APP_POWERON_TASK)
    {
        app_prev_task = APP_FM_TASK;
    }
...
///2020-12-4 获取上一个模式
u8 app_get_last_task()
{
    return app_prev_task;
}
