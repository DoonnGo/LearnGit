...
void bt_key_call_hand_up()
{
    ...
    bt_key_input_phone_number_backspace();///2020-11-28 输入号码退格,内有可操作判断条件
}
...
void bt_key_call_answer()
{
    ...
    {
        ///2020-11-28 确认输入号码并拨出
        bt_key_input_phone_callout();
    }
}
...
*/
/*----------------------------------------------------------------------------*/
void bt_key_input_phone_number(u8 number, u8 clear)
{
    if ((get_call_status() == BT_CALL_ACTIVE) ||
        (get_call_status() == BT_CALL_OUTGOING) ||
        (get_call_status() == BT_CALL_ALERT) ||
    ...
    if(clear)
    {
        memset(bt_user_priv_var.input_phone_num, 0, 30);
        bt_user_priv_var.input_phone_len = 0;
    }
    bt_user_priv_var.input_phone_num[bt_user_priv_var.input_phone_len] = number;
    bt_user_priv_var.input_phone_len++;
}
void bt_key_input_phone_number_backspace(void)
{
    if ((get_call_status() == BT_CALL_ACTIVE) ||
        (get_call_status() == BT_CALL_OUTGOING) ||
        (get_call_status() == BT_CALL_ALERT) ||
    ...
    if(bt_user_priv_var.input_phone_len == 0)
        return;
    bt_user_priv_var.input_phone_num[bt_user_priv_var.input_phone_len] = 0;
    bt_user_priv_var.input_phone_len--;
}
...
*/
/*----------------------------------------------------------------------------*/
void bt_key_input_phone_callout(void)
{
    if ((get_call_status() == BT_CALL_ACTIVE) ||
        (get_call_status() == BT_CALL_OUTGOING) ||
        (get_call_status() == BT_CALL_ALERT) ||
    ...
    user_send_cmd_prepare(USER_CTRL_DIAL_NUMBER, bt_user_priv_var.input_phone_len,
                        bt_user_priv_var.input_phone_num);
}
#endif
