...
#ifdef CONFIG_BOARD_AC695X_CHARGING_BIN
...
/************************** PWR config ****************************/
struct port_wakeup usbin_det_port = {
...
struct port_wakeup hall_det_port = {
    .pullup_down_enable = DISABLE,                            //配置I/O 内部上下拉是否使能
    .edge       = RISING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
    .iomap      = IO_PORTB_00,                             //唤醒口选择
};

struct port_wakeup key_port = {
...
#if WIRELESS_ENABLE
struct port_wakeup wl_det_port = {
    .pullup_down_enable = DISABLE,                            //配置I/O 内部上下拉是否使能
    .edge       = RISING_EDGE,                            //唤醒方式选择,可选：上升沿\下降沿
    .attribute  = BLUETOOTH_RESUME,                        //保留参数
...
struct wakeup_param wk_param = {
    .port[1] = &usbin_det_port,
    .port[2] = &hall_det_port,
    .port[3] = &key_port,
#if (WIRELESS_ENABLE)
    .port[4] = &wl_det_port,
#endif
...
const struct chargeIc_platform_data chargeIc_data = {
    .iic = 0,
    .chargeIc_name = "IC_SELF",
};

const struct io_det_platform_data hall_det_data = {
    .port = TCFG_HALL_PORT,
    .level = TCFG_HALL_OPEN_STATUS,
    .online_time = 10,
    .offline_time = 30,
   };
...
#if TCFG_NORFLASH_DEV_ENABLE
    extern u32 init_norsdfile_hdl(void);
    init_norsdfile_hdl();
#endif
...
#if TCFG_NOR_FS_ENABLE
    extern int nor_fs_ops_init(void);
    nor_fs_ops_init();
#endif
...
//根据需要进入休眠时设置相关的IO
void chgbox_enter_soft_power_off(void)
{
    u32 tt = 0;
    log_info("enter chgbox soft power off set\n");
    ///唤醒口的重设置
    if(gpio_read(USB_ONLE_DET_IO) && sys_info.status[USB_DET]==STATUS_ONLINE){
        usbin_det_port.edge = FALLING_EDGE;
    }else if(!gpio_read(USB_ONLE_DET_IO) && sys_info.status[USB_DET]==STATUS_OFFLINE){
        usbin_det_port.edge = RISING_EDGE;
    }

    if(gpio_read(TCFG_HALL_PORT) && sys_info.status[LID_DET]==STATUS_ONLINE){
        hall_det_port.edge = FALLING_EDGE;
    }else if(!gpio_read(TCFG_HALL_PORT) && sys_info.status[LID_DET]==STATUS_OFFLINE){
        hall_det_port.edge = RISING_EDGE;
    }

#if(WIRELESS_ENABLE)
    if(gpio_read(WL_AD_DET_IO) && sys_info.status[WIRELESS_DET]==STATUS_ONLINE){
        //当前无线充在线，关掉无线充的唤醒，免一直被唤醒
        wk_param.port[4] = NULL;
#if(DCDC_CTRL_EN==0)//无dcdcen,要关掉usb 唤醒，避免一直唤醒
        wk_param.port[1] = NULL;
#endif
    }
#endif
    ...
    //重新初始化唤醒口
    reset_the_wakeup_param(&wk_param);
...
#if TCFG_CHARGE_BOX_SOFT_POWER_OFF_IO_CTRL_ENABLE
    ////需要设置的IO
    gpio_set_pull_down(BOOST_CTRL_IO, 1);//5v升压en要低,开下拉

    gpio_set_pull_down(VOL_CTRL_IO, 1);//power lr en 要低,开下拉

    gpio_set_pull_down(VOR_CTRL_IO, 1);//power lr en 要低,开下拉

    gpio_set_output_value(CHGBOX_HANDSHAKE_IO,1); //握手需要拉高

#if(WIRELESS_ENABLE && DCDC_CTRL_EN)
    gpio_set_output_value(DCDC_EN_IO,0); //dcdc en 需要关闭
#endif
...
#endif//CONFIG_BOARD_AC695X_CHARGING_BIN
