...
const struct low_power_param power_param = {
	...
	.osc_type       = OSC_TYPE_LRC,//OSC_TYPE_BT_OSC ,
#if TCFG_USE_FAKE_RTC
	.fake_rtc  = 1,
#endif
};
...
struct port_wakeup port0 = {
    ...
    .iomap      = IO_PORTB_01,//IO_PORTA_10,                             //唤醒口选择
};
...
extern int devices_charge_init();
extern u8 get_dcdc_ctl_port(void);
void board_init()
{
    board_power_init();
    adc_vbg_init();
...
void board_set_soft_poweroff(void)
{
    ...
    if(TCFG_LOWPOWER_POWER_SEL == PWR_DCDC15){
        ...
        gpio_direction_output(power_param.dcdc_port, 0);
    }
