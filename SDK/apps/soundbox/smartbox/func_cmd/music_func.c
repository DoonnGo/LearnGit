...
u32 music_func_get(void *priv, u8 *buf, u16 buf_size, u32 mask)
{
    ...
    if (mask & BIT(MUSIC_INFO_ATTR_FILE_NAME)) {
        ...
        if (lfn_buf) {
            ...
            int unicode = fget_name_type(lfn_buf, lfn_len);
            ...
            /* printf("clust %x\n", clust); */
            u8 code_type = 0;
            if ((lfn_len <= 12) && (unicode == 0)) {
                code_type = 1;
            }

            /* printf("file name = %s\n", lfn_buf); */
            /* put_buf(lfn_buf, lfn_len); */
            ...
            offset += mucis_func_add_one_attr_continue(buf, buf_size, offset, MUSIC_INFO_ATTR_FILE_NAME, lfn_buf, lfn_len);
            free(lfn_buf);
        }
    }
