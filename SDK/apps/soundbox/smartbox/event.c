...
int smartbox_key_event_deal(u8 key_event, int ret)
{
    ...
    case KEY_REVERB_OPEN:
        if (ret) {
            extern void smartbox_update_reverbrateion_state_by_key(void);
            smartbox_update_reverbrateion_state_by_key();
            smartbox_function_update(COMMON_FUNCTION, BIT(COMMON_FUNCTION_ATTR_TYPE_MISC_SETTING_INFO));
        }
        break;
    }
    return ret;
