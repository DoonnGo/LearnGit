...
///获取需显示的台号
static u16 cur_fmrx_channel = 0;
bool fm_manage_set_channel(u8 channel)
{
    if (fm_hdl) {
        cur_fmrx_channel = channel;
        return 1;
    }
    return 0;
}

u16 fm_manage_get_channel()
{
    if (fm_hdl) {
        return  cur_fmrx_channel;
    }
    return 0;
}
