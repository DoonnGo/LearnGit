...
static int general_player_scandisk_break(void)
{
    ///注意：
...
int general_play_by_sculst(char *logo, u32 sclust)
{
    ...
    if (cur_logo == NULL || strcmp(cur_logo, logo) != 0) {
        ...
        __this->fsn = dev_manager_scan_disk(__this->dev, NULL, scan_parm, 0, general_player_scandisk_break);
        if (!__this->fsn) {
            return GENERAL_PLAYER_ERR_FSCAN;
        }
        __this->file = file_manager_select(__this->fsn, FSEL_BY_SCLUST, sclust);
        if (!__this->file) {
