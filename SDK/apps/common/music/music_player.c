...
#define MUSIC_PLAYER_CYCLE_ALL_DEV_EN					0//FCYCLE_ALL循环模式是否使能所有设备循环
...
static void music_player_mode_save_do(void *priv)
{
    local_irq_disable();
    if (++save_mode_cnt >= 5) {
        sys_hi_timer_del(save_mode_timer);
        save_mode_timer = 0;
        save_mode_cnt = 0;
        local_irq_enable();
        log_info("MODE_SAVE\n");
    ...
    if (__this) {
        ...
        if (__this->fsn) {
            ...
            if (save_mode_timer == 0) {
                //save_mode_timer = sys_hi_timer_add(NULL, music_player_mode_save_do, 1000);    ///2020-11-25 播放模式不记忆,有BUG
            }
            local_irq_enable();
...
///2020-11-25 Music set playmode
int User_music_set_playmode(char *mode)
{
    u8 play_mode;
    if(__this)
    {
        if(memcmp(mode, "int", 3) == 0)
            play_mode = FCYCLE_INT;
        else if(memcmp(mode, "rpt", 3) == 0)
            play_mode = FCYCLE_ONE;
        else if(memcmp(mode, "rdm", 3) == 0)
            play_mode = FCYCLE_RANDOM;
        else
            play_mode = FCYCLE_ALL;

        if(app_var.cycle_mode == play_mode)
            app_var.cycle_mode = FCYCLE_ALL;
        else
            app_var.cycle_mode = play_mode;
        return  music_player_set_repeat_mode(app_var.cycle_mode);
    }
    return -1;
}
int User_music_get_playmode(void)
{
    if(__this)
    {
        return app_var.cycle_mode;
...
int User_music_next_playmode(void)
{
    u8 play_mode;
    if(__this)
    {
        switch(app_var.cycle_mode)
        {
        case FCYCLE_INT:
            app_var.cycle_mode = FCYCLE_ONE;
            break;
        case FCYCLE_ONE:
            app_var.cycle_mode = FCYCLE_RANDOM;
            break;
        case FCYCLE_RANDOM:
            app_var.cycle_mode = FCYCLE_ALL;
            break;
...
///2020-11-14 MUSIC播放下10首
int User_music_play_prev_10(void)
{
    int err;
    u32 cur_file = music_player_get_file_cur();
    u32 totle_file =  music_player_get_file_total();
    if (music_player_get_record_play_status() == false)
    {
        if(cur_file <= 1)
        {
            err = music_player_play_by_number(NULL, totle_file);
        }
        else if(cur_file <= 10)
        {
            err = music_player_play_by_number(NULL, 1);
        }
        else
        {
            err = music_player_play_by_number(NULL, cur_file - 10);
        }
        return err;
    }
...
///2020-11-14 MUSIC播放下10首
int User_music_play_next_10(void)
{
    int err;
    u32 cur_file = music_player_get_file_cur();
    u32 totle_file =  music_player_get_file_total();
    if (music_player_get_record_play_status() == false)
    {
        if(cur_file >= totle_file)
        {
            err = music_player_play_by_number(NULL, 1);
        }
        else if((cur_file + 10) > totle_file)
        {
            err = music_player_play_by_number(NULL, totle_file);
        }
        else
...
///2020-11-27 Music KEY_NUMBER select music_number to play
static u16 User_music_number;
static u16 User_music_number_id = 0;
static void User_music_play_by_number(void)
{
    if(User_music_number_id == 0)
        return;

    User_music_number_id = 0;
    if(User_music_number <= music_player_get_file_total())
    {
        music_player_play_by_number(NULL, User_music_number);
    }
}

void User_music_number_put(u16 number)
{
    //if(UI_GET_CURR_MENU() != MENU_INPUT_NUMBER){}
    User_music_number *= 10;
    User_music_number += number;
    if(User_music_number > 9999)
        User_music_number %= 10000;

    if(User_music_number_id)
        sys_timer_re_run(User_music_number_id);
    else
        User_music_number_id = sys_timeout_add(NULL, User_music_play_by_number, 5000);
}

u16 User_get_music_number(void)
{
    return User_music_number;
}

void User_music_number_clear(void)
{
    User_music_number = 0;
    if(User_music_number_id)
    {
        sys_timeout_del(User_music_number_id);
    }
    User_music_number_id = 0;
}
...
int music_player_play_first_file(char *logo)
{
    if (logo == NULL) {
        ...
        __this->fsn = dev_manager_scan_disk(__this->dev, NULL, scan_parm, app_var.cycle_mode, __this->parm.cb->fsn_break);
    }
    if (__this->fsn == NULL) {
        return MUSIC_PLAYER_ERR_FSCAN;
    }
...
int music_player_play_devcie_prev(struct __breakpoint *bp)
{
    ...
    ///get fscan
    __this->fsn = dev_manager_scan_disk(__this->dev, NULL, (const char *)scan_parm, app_var.cycle_mode, __this->parm.cb->fsn_break);
    if (__this->fsn == NULL) {
        return MUSIC_PLAYER_ERR_FSCAN;
    }
    int err = 0;
...
int music_player_play_record_folder(char *logo, struct __breakpoint *bp)
{
    ...
    ///需要扫盘
    struct vfscan *fsn = dev_manager_scan_disk(dev, NULL, scan_parm, app_var.cycle_mode, __this->parm.cb->fsn_break);
    if (fsn == NULL) {
    ...
    ///get file
    if (bp) {
        __this->file = file_manager_select(__this->fsn, FSEL_BY_SCLUST, bp->sclust);
        if (__this->file == NULL) {
            return MUSIC_PLAYER_ERR_FILE_NOFOUND;
        }
        ///start decoder
