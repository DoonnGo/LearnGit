...
///2020-11-27 清空VM中的断点信息
void breakpoint_clear(void)
{
    struct __breakpoint *breakpoint = NULL;

    breakpoint = breakpoint_handle_creat();
    memset(breakpoint, 0, sizeof(struct __breakpoint) + BREAKPOINT_DATA_LEN);

    vm_api_write_mult(CFG_SD0_BREAKPOINT0, CFG_SD0_BREAKPOINT0 + 10, breakpoint, BREAKPOINT_VAILD_SIZE(breakpoint), 2);
    vm_api_write_mult(CFG_SD1_BREAKPOINT0, CFG_SD1_BREAKPOINT0 + 10, breakpoint, BREAKPOINT_VAILD_SIZE(breakpoint), 2);
    vm_api_write_mult(CFG_USB_BREAKPOINT0, CFG_USB_BREAKPOINT0 + 10, breakpoint, BREAKPOINT_VAILD_SIZE(breakpoint), 2);
