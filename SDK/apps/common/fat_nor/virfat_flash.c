...
static u32 virfat_flash_read(void *buf, u32 addr_sec, u32 len)
{
    ...
    if (addr_sec == 0) {
        ...
        real_addrsec = addr_sec - virfat_flash.fatbase + 0; // 0为flash里面fat表实际位置。
    } else if (addr_sec < virfat_flash.database) {
        real_addrsec = addr_sec - virfat_flash.dirbase + (4 * 1024) / 512; // 4K为flash里面目录项实际位置。
    } else {
        real_addrsec = addr_sec - virfat_flash.database + 8 * 1024 / 512; // 8K为flash里面数据区实际位置。
    }
