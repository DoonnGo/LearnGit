...
u16 uac_get_mic_vol(const usb_dev usb_id)
{
    if (uac_info) {
        if (!uac_info->mic_mute) {
            return vol_convert(uac_info->mic_vol) / 2 + 6;
        }
    }
    return 0;
}
...
static void spk_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    usb_g_iso_read(usb_id, SPK_ISO_EP_OUT, NULL, SPK_FRAME_LEN, 0);
...
static void mic_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    const u32 mic_frame_len = ((mic_samplingfrequency * MIC_AUDIO_RES / 8 * MIC_CHANNEL) / 1000);
